    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                init_s = 0.5 * (init_s + 1.)
                init_s_input = s_vae_p(images=init_s, return_tensors="pt", padding=True, do_rescale=False).to(device)
                init_s = s_vae_m.get_image_features(**init_s_input)
            ...
            # loss_dict = diffusion.training_losses_long(model, x, t, x_prev, init_s, model_kwargs)
